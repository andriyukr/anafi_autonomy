...
# All SKYCONTROLLERs have a default IP address set to 192.168.53.1. Hence by default,
# multiple SKYCONTROLLERs connected to a host via multiple USB ports are not
...
# routed to the associated SKYCONTROLLER.
    
# USAGE:
#     setup:
#         $ ./config_skycontroller_ip.sh setup wlan0 192.168.54.1 201
#         $ ./config_skycontroller_ip.sh setup wlan1 192.168.55.1 202
#         $ ./config_skycontroller_ip.sh setup wlan2 192.168.56.1 203
#     cleanup:
#         $ ./config_skycontroller_ip.sh cleanup wlan0 192.168.54.1 201
#         $ ./config_skycontroller_ip.sh cleanup wlan1 192.168.55.1 202
#         $ ./config_skycontroller_ip.sh cleanup wlan2 192.168.56.1 203
...
# Outbound traffic iptables rules: packets addressed to the virtual SKYCONTROLLER's IP
# address are marked in order to be routed on the correct interface and then get
# NATed to match the actual SKYCONTROLLER's IP address on this interface
OUT_RULES=$(cat <<EOF 
...
# only NATing the traffic from the SKYCONTROLLER interface to the virtual SKYCONTROLLER IP address
